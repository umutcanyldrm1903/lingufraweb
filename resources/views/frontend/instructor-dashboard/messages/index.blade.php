...
@section('dashboard-contents')
    <div class="sm-header">
        <div>
            ...
            <p class="sm-subtitle">{{ __('Chat with your students and manage your messages here.') }}</p>
        </div>
    </div>

    <div class="sm-grid">
        <div class="sm-list">
            <h6 class="sm-list__title">{{ __('Students') }}</h6>
            <div class="sm-contacts">
                @forelse ($students as $student)
                    @php
                        $active = $partner && $partner->id === $student->id;
                        $lastMessage = $threads[$student->id] ?? null;
                        $unread = $unreadCounts[$student->id] ?? 0;
                    @endphp
                    <a href="{{ route('instructor.messages.index', $student->id) }}" class="sm-contact {{ $active ? 'is-active' : '' }}">
                        <div class="sm-contact__avatar">
                            <img src="{{ asset($student->image ?? 'frontend/img/placeholder/instructor.png') }}" alt="{{ $student->name }}">
                        </div>
                        <div class="sm-contact__meta">
                            <div class="sm-contact__name">{{ $student->name }}</div>
                ...
                @empty
                    <p class="text-muted">{{ __('No students yet.') }}</p>
                @endforelse
            </div>
        </div>
        ...
        <div class="sm-thread">
            @if ($partner)
                ...
                <form class="sm-form" method="POST" action="{{ route('instructor.messages.store', $partner->id) }}">
                    @csrf
            ...
            @else
                <div class="sm-empty">
                    <p class="sm-empty__text">{{ __('Select a student to start a chat.') }}</p>
                </div>
            @endif
        </div>
    </div>
